...
- **HTTPS**: the TLS handshake is intercepted; the certificate presented is
  `CN=*.newrozholdings.com, O=Allai Newroz Telecom, L=Erbil`, which belongs to
  the ISP's FortiGate firewall, not the real site. Verification fails on any
  client.
- **HTTP**: returns a captive-portal JavaScript redirect to
  `https://hq-74-gateway.newrozholdings.com:1003/fgtauth?...` (a FortiGate
  authentication gateway) instead of site content.
- Applies to both IPv4 (Cloudflare edge 188.114.x.x) and IPv6. The
  interception is SNI-based, so changing DNS does not help.
...
cloaking malware, which is worth reporting to the University of Duhok IT
staff.
...
The server answers bursts with HTTP 429 (the first run died on it). Reducing
to 2 workers with a 1s delay worked reliably. The crawl was later stopped
intentionally after ~3,600 pages because the site exposes no documents
(HTML-only articles); a full crawl of its ~18.7k sitemap URLs would take
hours for no document yield.

## 4. spirez.org: most PDFs are image-only scans (data-quality blocker)

About 76% of sampled spirez PDFs (est. ~1,730 of 2,291) have no text layer.
The files are downloaded and valid, but OCR is required before their text
(potentially 5 to 14M Badini words) can be used for training.

## 5. Minor, expected errors (no action needed)

- **zcks**: 118 x HTTP 404, dead links on the site itself.
- **govarabadinan**: Blogger's robots.txt disallows `/search` pagination
  (respected); a few post links with unencoded spaces 404, which are broken
  links on the blog itself. All posts were still reached via the sitemap.
