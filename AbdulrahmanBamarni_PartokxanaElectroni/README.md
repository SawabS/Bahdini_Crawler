...
PDF collection scraped from the **Partokxana Electroni** ("Electronic Library")
Facebook group, gathered as Kurdish text data for the
[Kurdish LLM fine-tuning project](../../../kurdish_llm_finetuning_plan.md)
...
| Path | What it is |
|---|---|
| `facebook_pdf_downloader.py` | The crawler (Playwright + requests). See below. |
| `pdfs/` | Downloaded PDF files. *(created on first run)* |
| `permalinks.json` | Cached list of post IDs harvested from the files tab. *(created on first run)* |
| `manifest.json` | Per-post download log: status, filename, size, errors. This is what makes runs resumable. *(created on first run)* |
| `.fb_profile/` | Persistent Chromium profile holding the Facebook session, so you only log in once. **Contains auth cookies — do not share or commit.** *(created on first run)* |

## How to run

Requires the `ai` conda environment (already has `playwright 1.61`, `requests`,
`tqdm`, and the Chromium browser installed).

```bash
cd "/home/sawab/AI - Project/KI_finetuning/data/AbdulrahmanBamarni_PartokxanaElectroni"
...
- Downstream processing (text extraction) can follow the same pattern as
  [`../../Metin/text_extractor.py`](../../Metin/text_extractor.py).
